...
def get_virtual_instrument(noise: float = 1e-1, hull_tracing_ratio: float = 0.25,
        boundary_dataset_path: str = str(DATA_DIR / "phase_data/tutorial.nc"),
        reference_data_path: str = str(DATA_DIR / "SANS"), ) -> VirtualSAS:
    ...
    for fname in ['low_q.ABS', 'med_q.ABS', 'high_q.ABS']:
        ...
        inst_client.add_configuration(q=list(data.q), I=list(data.I), dI=list(data.dI), dq=list(data.dq), reset=False)
    inst_client.add_sasview_model(label='L2', model_name='sphere',
        model_kw={'scale': 0.005, 'background': 1.0, 'sld': 1.0, 'sld_solvent': 6.0, 'radius': 200, })

    inst_client.add_sasview_model(label='D', model_name='power_law',
